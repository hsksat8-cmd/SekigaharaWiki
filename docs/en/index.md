# Sekigahara: What-If Battle Simulator Wiki

<div class="hero" markdown="1">

This is the official wiki for **Sekigahara: What-If Battle Simulator**, a game in which you change formations, troop strength, morale, and commander behavior, then observe how the battle develops.

[View on Microsoft Store](https://apps.microsoft.com/detail/9NB8RVSRWS1F?hl=en-us&gl=US){ .md-button .md-button--primary }
[Development posts on X](https://x.com/IFHistorySim){ .md-button }
[Watch on YouTube](https://www.youtube.com/@IFHistorySim){ .md-button }
[日本語版](../){ .md-button }

</div>

![Tokugawa Ieyasu visual](../assets/images/tokugawa-ieyasu-portrait.jpg){ .wiki-image }
<p class="image-caption">Tokugawa Ieyasu visual from an X post</p>

## Main Features

<div class="stat-grid">
  <div class="stat-card"><strong>Autonomous units</strong><br>Units act according to morale, position, battlefield conditions, and other factors.</div>
  <div class="stat-card"><strong>What-if scenarios</strong><br>Test alternate outcomes such as Kobayakawa remaining neutral or joining the Western Army.</div>
  <div class="stat-card"><strong>Five formations</strong><br>Square, Flexible Square, Crane Wing, Deep, and Fish Scale.</div>
  <div class="stat-card"><strong>Observe or command</strong><br>Watch the battle or command the Eastern Army, Western Army, or player-created commanders.</div>
</div>

## Recommended Starting Pages

- [Game Overview](game/overview.md)
- [Play Modes](gameplay/modes.md)
- [Battle System](systems/index.md)
- [Formation Overview](formations/index.md)
- [Scenario Overview](scenarios/index.md)
- [Formation Experiment: Deep vs. Square](experiments/deep-vs-square.md)
- [Image Gallery](reference/gallery.md)

## About This Wiki

This wiki is based on the archived posts of the X account `@IFHistorySim`. It reorganizes confirmed game specifications and development notes by topic without filling gaps with unsupported assumptions.
